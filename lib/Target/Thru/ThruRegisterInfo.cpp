...
static cl::opt<bool>
ReserveAppRegisters("Thru-reserve-app-registers", cl::Hidden, cl::init(false),
                    cl::desc("Reserve application registers (%g2-%g4)"));

ThruRegisterInfo::ThruRegisterInfo() : ThruGenRegisterInfo(SP::O7) {}
...
const uint32_t*
ThruRegisterInfo::getRTCallPreservedMask(CallingConv::ID CC) const {
  return RTCSR_RegMask;
}

BitVector ThruRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  BitVector Reserved(getNumRegs());
  const ThruSubtarget &Subtarget = MF.getSubtarget<ThruSubtarget>();
...
ThruRegisterInfo::getPointerRegClass(const MachineFunction &MF,
                                      unsigned Kind) const {
  const ThruSubtarget &Subtarget = MF.getSubtarget<ThruSubtarget>();
...
void
ThruRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  // const ThruSubtarget &Subtarget = MF.getSubtarget<ThruSubtarget>();
  // const ThruFrameLowering *TFI = getFrameLowering(MF);
...
// Thru has no architectural need for stack realignment support,
// except that LLVM unfortunately currently implements overaligned
// stack objects by depending upon stack realignment support.
// If that ever changes, this can probably be deleted.
bool ThruRegisterInfo::canRealignStack(const MachineFunction &MF) const {
  if (!TargetRegisterInfo::canRealignStack(MF))
    return false;

  // Thru always has a fixed frame pointer register, so don't need to
